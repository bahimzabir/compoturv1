...
function parseTerms(expression: string): Term[] {
  ...
  if (processedExpression.length > 0 && processedExpression[0] !== '+' && processedExpression[0] !== '-') {
    processedExpression = '+' + processedExpression;
  }
  
  while (currentIndex < processedExpression.length) {
    // Look for the start of the next term (+ or -)
    if (processedExpression[currentIndex] === '+' || processedExpression[currentIndex] === '-') {
      // If not the first term, process the previous term
      if (currentIndex > 0) {
        const termString = processedExpression.substring(startIndex, currentIndex).trim();
        if (termString) {
